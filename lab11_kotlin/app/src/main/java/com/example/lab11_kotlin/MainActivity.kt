...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        dbrw = MyDBHelper(this).getReadableDatabase()
        ...
        btn_insert!!.setOnClickListener {
            if (ed_book!!.length() < 1 || ed_price!!.length() < 1)
                Toast.makeText(this@MainActivity, "欄位請物留空", Toast.LENGTH_SHORT).show()
            else {
                try {
                    dbrw!!.execSQL(
        ...
        btn_update!!.setOnClickListener {
            if (ed_book!!.length() < 1 || ed_price!!.length() < 1)
                ...
                try {
                    dbrw!!.execSQL("UPDATE myTable SET price = " + ed_price!!.text.toString() +
                            " WHERE book LIKE '" + ed_book!!.text.toString() + "'")
                    Toast.makeText(
                        this@MainActivity,
        ...
        btn_delete!!.setOnClickListener {
            if (ed_book!!.length() < 1)
                Toast.makeText(this@MainActivity, "書名請物留空", Toast.LENGTH_SHORT).show()
            else {
                try {
